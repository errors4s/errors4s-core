  ...
  def stringContextMacro(c: Context)(args: c.Expr[Any]*): c.Expr[NonEmptyString] = {
    ...
    c.prefix.tree match {
      case Apply(_, List(Apply(_, scArgs))) if scArgs.exists(isNonEmptyLiteralString(c)) =>
        c.Expr(
          q"""org.errors4s.core.NonEmptyString.from(StringContext(${scArgs}: _*).s(..${args})).getOrElse(throw new AssertionError("Error during NonEmptyString macro expansion. This is an errors4s bug, please report it."))"""
        )
      case Apply(_, List(_)) =>
        c.abort(
          c.enclosingPosition,
