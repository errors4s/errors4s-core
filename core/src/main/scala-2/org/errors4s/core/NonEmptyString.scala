  ...
  def make(c: Context)(value: c.Expr[String]): c.Expr[NonEmptyString] = {
    ...
    value.tree match {
      case Literal(Constant(value: String)) if value.size > 0 =>
        c.Expr(
          q"""_root_.org.errors4s.core.NonEmptyString.from($value).getOrElse(throw new AssertionError("Error during macro expansion of NonEmptyString. This is a bug in errors4s-core. Please report it."))"""
        )
      case _ =>
        c.abort(
          c.enclosingPosition,
          "NonEmptyString.apply can only be used with non-empty String literal values. Consider using the `nes` interpolator which is more flexible. You can access by importing org.errors4s.core.syntax.all._"
        )
    }
  }
}
