...
/** An error type which is guaranteed to be useful.
  ...
  *
  *   - Create a custom Error ADT and then convert it to [[java.lang.Throwable]] when needs be (which is not always the
  *     case).
  *   - Extend [[java.lang.RuntimeException]] (or sometimes [[java.lang.Exception]] ) and raise/throw the error.
  *   - Create a `new RuntimeException(<message>)` and raise/throw that value directly.
  *
  * [[Error]] aims to support all these cases.
  *
  * It is an open `trait`, so custom Error ADTs can extend it. This gives error handling code the ability to be generic
  ...
  * or you can just create instances of [[Error]] directly and raise them, similar to `new RuntimeException(<message>).
  *
  * [[Error]] also supports annotating [[Error#causes]], similar to [[java.lang.Throwable#getCause]], but more
  * expressive. Rather than having a single `cause` (which may be `null`) [[Error]] allows for an arbitrary number of
  * causes.
  *
...
trait Error extends RuntimeException {
  ...
  /** The primary error message for contexts in which a single value is required, e.g.
    * [[java.lang.Throwable#getMessage]].
    *
    * For code which understands [[org.errors4s.core.Error]] it is recommended to use [[#errorMessages]] instead. This
    * gives you ''all'' the errors.
    */
  def primaryErrorMessage: NonEmptyString

  /** A set of secondary error messages. Often these are values interpolated from the given context. This is why they
    * are [[java.lang.String]] values and not `cats.data.NonEmptyString` values.
    */
  def secondaryErrorMessages: Vector[String] = Vector.empty

  /** A set of causes for this [[org.errors4s.core.Error]]. The first value in this set, if any, will be the value
    * returned by [[org.errors4s.core.Error#getCause]].
    */
  def causes: Vector[Throwable] = Vector.empty

  /** An error message for each values in [[#causes]]. This is just a convenience method for aggregating all error
    * messages.
    */
  final lazy val causesErrorMessages: Vector[String] = causes.map(value => Error.errorMessageFromThrowable(value).value)

  /** All the error messages for this [[org.errors4s.core.Error]]. This includes the [[#primaryErrorMessage]], the
    * [[#secondaryErrorMessages]], and all error messages from [[#causesErrorMessages]].
    */
...
object Error {
  ...
  /** Create an [[Error]] from an error message, a set of secondary error messages, and a set of [[java.lang.Throwable]]
    * causes.
    */
  ...
  /** Create an [[Error]] from an arbitrary [[java.lang.Throwable]].
    *
    * @note
    *   Use of this method is discouraged unless you are in a situation where you don't have any context at all.
    *   Otherwise use [[#withMessageAndCause]].
    */
  ...
  /** Generate a `NonEmptyString` error message from any given [[java.lang.Throwable]].
    *
    * This is first attempt to use [[java.lang.Throwable#getLocalizedMessage]], falling back to the
    * [[java.lang.Class#getCanonicalName]], then [[java.lang.Class#getName]], finally if either of those yield the empty
    * [[java.lang.String]] (which should be impossible), a default `NonEmptyString` is used.
    */
  ...
  /** Get the name of anything.
    *
    * @note
    *   only use this function to attempt to provide information about a problem if everything else has failed. That is,
    *   if at all possible don't use this method. It is intended to help generate useful information about an error in
    *   the case where no other useful information was provided, e.g. when `Throwable.getLocalizedMessage` is `null`.
    */
