...
private[scalacheck] trait NonEmptyStringInstances {
  ...
  implicit final def arbNonEmptyString(implicit arbString: Arbitrary[String]): Arbitrary[NonEmptyString] =
    Arbitrary(
      arbString
        .arbitrary
  ...
  implicit final def cogenNonEmptyString(implicit cogenString: Cogen[String]): Cogen[NonEmptyString] =
    cogenString.contramap(_.value)
}
